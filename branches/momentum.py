...
class Momentum():
    def __init__(self, dname):
        self.dname = dname
        self.aname = "_momentum" #a for analysis
        self.sname = ""
    ...
    def profile(self, fnum, bar_widths=[20,30,20]):
        now = datetime.now()
        self.sname = ""
        ...
        ax_momr = fig.add_subplot(gs[0, 1])
        ax_momrv = fig.add_subplot(gs[1, 1])

        ax_momphi = fig.add_subplot(gs[0, 2])
        ax_momphiv = fig.add_subplot(gs[1, 2])

        ax_momz = fig.add_subplot(gs[0, 3])
        ax_momzv = fig.add_subplot(gs[1, 3])

        aa.midplane_colorplot(aa.rho, ax_rho, plot_COM=False, vbound=[1e-5,1e2], slicetype='z', rotation=rotation)
        aa.midplane_colorplot(aa.rho, ax_rhov, plot_COM=False, vbound=[1e-5,1e2], slicetype='vert', rotation=rotation)
        ax_rho.set_title("Density")

        aa.midplane_colorplot(aa.rho * aa.vel_r, ax_momr, log=False, vbound=[-bar_widths[0],bar_widths[0]], slicetype='z', rotation=rotation, cmap="PRGn")
        aa.midplane_colorplot(aa.rho * aa.vel_r, ax_momrv, log=False, vbound=[-bar_widths[0],bar_widths[0]], slicetype='vert', rotation=rotation, cmap="PRGn")
        ax_momr.set_title("Radial Momentum")

        v_phi = aa.vel_phi - (np.sqrt(sim.gm1/aa.r)) + aa.r*sim.orbital_Omega
        aa.midplane_colorplot(aa.rho * v_phi, ax_momphi, log=False, vbound=[-bar_widths[1],bar_widths[1]], slicetype='z', rotation=rotation, cmap="PRGn")
        aa.midplane_colorplot(aa.rho * v_phi, ax_momphiv, log=False, vbound=[-bar_widths[1],bar_widths[1]], slicetype='vert', rotation=rotation, cmap="PRGn")
        ax_momphi.set_title("Azimuthal Momentum - Kepler")

        aa.midplane_colorplot(aa.rho * aa.vel_z, ax_momz, log=False, vbound=[-bar_widths[2],bar_widths[2]], slicetype='z', rotation=rotation, cmap="PRGn")
        aa.midplane_colorplot(aa.rho * aa.vel_z, ax_momzv, log=False, vbound=[-bar_widths[2],bar_widths[2]], slicetype='vert', rotation=rotation, cmap="PRGn")
        ax_momz.set_title("Vertical Momentum")
